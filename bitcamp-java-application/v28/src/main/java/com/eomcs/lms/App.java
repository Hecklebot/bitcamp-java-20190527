  ...
  private static void saveLessonData() {
    ...
    try {
      ...
      try {
        ...
        // close() 하다 발생한 예외는 따로 처리할 필요가 없다. 따라서 catch 블록은 빈 채로 둔다.
      }
    }
  }
    ...
    try {
      ...
      for (Member member : memberList) {
        ...
        String str = String.format("%d,%s,%s,%s,%s,%s,%s\n", member.getNo(), member.getName(),
            member.getPassword(), member.getEmail(), member.getTel(), member.getPhoto(), member.getRegisteredDate());
        out.write(str);
      }
