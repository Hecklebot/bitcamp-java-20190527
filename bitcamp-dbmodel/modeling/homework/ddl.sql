...
-- 교실사진
DROP TABLE IF EXISTS room_photo RESTRICT;
...
CREATE TABLE students (
  ...
  working    INTEGER      NOT NULL COMMENT '재직 여부' -- 재직 여부
)
COMMENT '학생';
...
CREATE TABLE lectures (
  lecture_id INTEGER      NOT NULL COMMENT '강의 번호', -- 강의 번호
...
ALTER TABLE lectures
  MODIFY COLUMN lecture_id INTEGER NOT NULL AUTO_INCREMENT COMMENT '강의 번호';
...
CREATE TABLE teachers (
  ...
  ncs_score  INTEGER      NULL     COMMENT 'NCS 점수' -- NCS 점수
)
COMMENT '강사';
...
CREATE TABLE centers (
  ...
  bas_addr  VARCHAR(255) NULL     COMMENT '기본 주소', -- 기본 주소
  det_addr  VARCHAR(255) NOT NULL COMMENT '상세 주소' -- 상세 주소
)
COMMENT '교육센터';
...
ALTER TABLE room_photo
  ADD CONSTRAINT PK_room_photo -- 교실사진 기본키
    PRIMARY KEY (
      photo_id -- 교실사진번호
    );
...
ALTER TABLE subjects
  ADD CONSTRAINT PK_subjects -- 강의 과목 기본키
    PRIMARY KEY (
      subject_id -- 강의과목번호
    );
...
CREATE TABLE members (
  ...
  post_no     CHAR(6)      NULL     COMMENT '우편번호', -- 우편번호
  bas_addr    VARCHAR(255) NULL     COMMENT '기본 주소', -- 기본 주소
  det_addr    VARCHAR(255) NULL     COMMENT '상세 주소', -- 상세 주소
...
-- 교실사진
ALTER TABLE room_photo
  ADD CONSTRAINT FK_rooms_TO_room_photo -- 교실 -> 교실사진
    FOREIGN KEY (
      room_id -- 교실번호
    )
    REFERENCES rooms ( -- 교실
...
-- 강사강의과목
ALTER TABLE teacher_subject
  ADD CONSTRAINT FK_subjects_TO_teacher_subject -- 강의 과목 -> 강사강의과목
    FOREIGN KEY (
      subject_id -- 강의과목번호
    )
    REFERENCES subjects ( -- 강의 과목
      subject_id -- 강의과목번호
    );
