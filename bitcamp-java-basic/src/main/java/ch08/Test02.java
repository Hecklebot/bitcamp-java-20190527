...
public class Test02 {
  public static void main(String[] args) {
    ...
    //클래스에 대해 레퍼선스 변수를 만들 때는 클래스를 로딩하지 않는다.
    My2 temp;
    
    Scanner keyScan = new Scanner(System.in);
    System.out.println("> ");
    keyScan.nextLine(); //사용자가 LineFeed(LF; 0x0A) -> enter 값을 입력해야만 리턴한다.
    
    //My2클래스를 사용할 때 My2.class 파일이 Method area에 로딩된다.
    //이 것을 테스트하려면 이 클래스를 실행하기 전에 My2.class를 삭제하라.
    //그리고 이 클래스를 실행하려면 위의 코드를 실행할 때는 오류가 발생하지 않는다.
    //그러나 아래의 코드를 실행할 때 My2.class가 없다고 NoClassFoundException이 발생한다.
