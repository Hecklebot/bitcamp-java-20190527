...
public class Test02 {
  ...
  public static void main(String[] args) {
    ...
    A obj4 = new My2(); 
    obj4.m1();              //m1()은 A 클래스에 있는 메서드이다. 호출할 수 있다. 다만 obj4에 저장된 인스턴스는 실제 My2 클래스이기 때문에
                            //m1() 메서드를 찾을 때는 My2 클래스부터 상위 클래스로 찾아 올라간다.
    
    obj4.m2("림꺽정");        //m2()또한 m1()과 같다.
    obj4.m2("임꺽정");
//  obj4.m3();              //컴파일 오류. obj4는 A 클래스의 레퍼런스이기 때문에, 문법적으로 m3() 메서드가 없어서 호출할 수 없다.
                            //obj4가 어떤 클래스의 인스턴스를 가리키는 가는 중요한 것이 아니다. 메서드를 호출할 때 레퍼런스의 클래스 조건에 맞는 메서드만 호출할 수 있다.
    
    //obj4에 My2 인스턴스를 저장한 것이 확실하다면, 컴파일러에게 개발자가 직접 알려줘야 한다.
    //즉, 다음과 같이 해당 레퍼런스에 어떤 클래스의 인스턴스가 들어 있는지 알려주기 위해
    //Type casting(형변환)해 줘야한다.
    ((My2) obj4).m3(); // obj4가 가리키는 객체가 어떤 타입인지 컴파일러에게 알려주면
                       // 그 클래스의 메서드를 호출할 수 있다.
                       // 레퍼런스만 형변환 하도록 괄호처리 하라.
  }
