// 바이트 배열에 출력할 때, Stream API를 사용하여 데이터 출력하기
...
public class Test01_3 {
  public static void main(String[] args) throws Exception {
    ...
    // 이를 해결하기 위해 나온 방법이 메모리에 출력할 때도 Stream API를 사용하는 것이다.

    // 1. 값을 메모리에 출력할 수 있도록 바이트 배열 출력 스트림을 준비한다.
    try (ByteArrayOutputStream out = new ByteArrayOutputStream()) {

      // 2. 짝수를 바이트 배열에 출력한다. 파일과 출력하는 과정과 같다.
      // -> 단, FileOutputStream과 달리 write()를 호출하여 값을 출력하면
      // -> 그 값은 ByteArrayOutputStream 객체 내부에 보관된다.
      for (int i = 2; i <= 100; i++) {
        if (i % 2 == 0) {
          out.write(i);
        }
