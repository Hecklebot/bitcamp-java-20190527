...
public class SqlSessionFactoryProxy implements SqlSessionFactory {
  ...
  // 스레드 전용 바구니를 준비한다. 이 바구니에는 SqlSession을 보관한다.
  ...
  //    이 전에 사용했던 SqlSession을 제거해야 한다.
  //    왜? 이 전에 사용한 SqlSession은 트랜잭션을 종료하는 순간 close()되었기 때문이다.
  //    쓰고 싶어도 쓸 수 없다.
  ...
  public SqlSession openSession() {
    SqlSession sqlSession = localSqlSession.get();
    // 스레드에 SqlSession이 들어 있으면(트랜잭션이 시작된 상태) 그 것을 그대로 리턴한다.
    if(sqlSession != null) {
      return sqlSession;
    }
