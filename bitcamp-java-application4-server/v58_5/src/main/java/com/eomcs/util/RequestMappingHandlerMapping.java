...
public class RequestMappingHandlerMapping {
  ...
  private static final Logger logger = LogManager.getLogger(RequestMappingHandlerMapping.class);
  ...
  public RequestMappingHandlerMapping(ApplicationContext iocContainer) {
    ...
    // 각 객체에서 @RequestMapping가 붙은 메서드를 꺼낸다.
  ...
  public static class RequestHandler {
    ...
    public Object invoke(HttpServletRequest request, HttpServletResponse response)
      ...
      // requestHandler를 호출하면, model 객체에는 requestHandler가 담은 값이 보관되어 있다.
      // 여기에 requestHandler의 리턴값(JSP url)도 함께 보관한다.
      model.put("viewUrl", viewUrl);

      // requestHandler의 작업 결과물과 JSP url을 담은 맵 객체를 프론트 컨트롤러에게 전달한다.
      return model;
    }

    private Object[] getArguments(
        Parameter[] params, HttpServletRequest request, 
    ...
    private List<String> getParameterNames(Class<?> type, Method method) throws Exception {
      // -> java.reflection API로는 알아낼 수 없다.(컴파일 옵션으로 가능)
      // -> 그러나 .class 파일에는 파라미터 이름이 들어있다.
      // -> 그 것을 꺼내려면 외부 라이브러리를 사용해야 한다.
      Reflections reflections = new Reflections(type, new MethodParameterNamesScanner());
    ...
    private Object getArgument(
        String paramName, Parameter param, HttpServletRequest request, 
        HttpServletResponse response, Map<String, Object> model)
            throws Exception {
      ...
      } else if (paramType == ServletResponse.class || paramType == HttpServlet.class) {
        return response;
    ...
    private Object getPojoValue(String paramName, Class<?> paramType, HttpServletRequest request)
      ...
      logger.debug(String.format("%s의 set()", paramType.getName()));

      // 클라이언트가 보낸 데이터 중, 세터 메서드에 넘겨줄 요청 데이터가 있다면 호출한다. 
      for (Method m : methods) {

        // 메서드 이름에서 프로퍼티 명을 추출하고 첫 알파벳을 소문자로 만든다.
        ...
        if (isPrimitiveType(m.getParameters()[0].getType())) {
          // 클라이언트가 보낸 값 중에 프로퍼티 이름과 일치하는 데이터가 있다면 꺼낸다.
          value = getPrimitiveValue(propName, m.getParameters()[0].getType(), request);
        }

        // 세터에 넘겨 줄 값을 준비하지 못 했으면 다음 메서드로 넘어간다.
        if (value == null) {
          continue;
        }
