...
// 서버 쪽에 BoardDao를 대행할 프록시 객체를 정의한다.
// -> 클라이언트는 이 프록시 객체를 사용하여 서버쪽의 BoardDao 객체를 사용할 것이다.
// -> 보통 서비스를 제공하는 서버 쪽에서 프록시 객체를 만들어 클라이언트 개발자에게 배포한다.
// -> 이런 방식으로 프로그램을 개발할 때 이점은 클라이언트 개발자가 서버와 어떻게 통신해야 하는지 알 필요가 없다는 것이다.
// -> 즉, 서버쪽과 통신하는 코드를 캡슐화하여 감추고, 메서드를 통해 단순화하는 기법이다.
...
public class LessonDaoProxy implements LessonDao {
  ...
  public static void main(String[] args) throws Exception {
    ...
    // System.out.println(lessonDao.findBy(100));
