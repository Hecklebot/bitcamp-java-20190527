...
public class Controller01_1 {
  ...
  @GetMapping("h1") // 뷰 이름을 리턴할 때는 @ResponseBody를 붙이면 안된다. -> 붙이면 클라이언트에게 표시함
  public String handler1(Model model) {
    ...
    // 기본 View Resolver는 리턴 값으로 URL을 받아
    // 웹 애플리케이션 디렉토리에서 JSP를 찾는다.
    // 웹 애플리케이션 경로가 /bitcamp-java-spring-webmvc 라면, JSP 경로는 다음과 같다.
  ...
  public String handler3(Map<String, Object> map) {
    ...
    // MVC 모델에서 JSP는 뷰 컴포넌트로서 출력이라는 역할을 담당한다.
    // 출력할 데이터를 준비하는 일은 페이지 컨트롤러가 담당한다.
    // 그래서 JSP를 실행할 때는 항상 페이지 컨트롤러를 통해 실행해야 한다.
    // 페이지 컨트롤러가 하는 일이 없어도 프로그램의 일관성을 위해 직접 JSP를 요청하지 않고,
    // 페이지 컨트롤러를 통해 요청해야 한다.
    // 
    // 그런데 웹 디렉토리에 JSP를 두면 클라이언트에서 JSP를 직접 요청할 수 있다.
    // 페이지 컨트롤러를 경유하지 않은 상태에서 실행해 봐야 소용없지만,
    // 그래도 요청은 할 수 있다.
    // 이런 의미없는 요청을 막는 방법으로, JSP 파일을 /WEB-INF/에 두는 것을 권장한다.
    //
    // 웹 브라우저에서 다음 URL의 JSP를 요청해 보라
    // 1) http://localhost:8888/bitcamp-java-spring-webmvc/jsp/c01_1.jsp         --> 됨
    //  -> 클라이언트가 요청할 수 있다.
    // 2) http://localhost:8888/bitcamp-java-spring-webmvc/WEB-INF/jsp/c01_1.jsp --> 안됨
    //  -> 클라이언트가 요청할 수 없다.
    //  -> /WEB-INF 폴더에 있는 자원들은 클라이언트에서 직접 요청할 수 없다.
    //  -> 그래서 잘못된 요청을 막을 수 있고, 실무에서는 이 방법을 사용한다.
  }
}
