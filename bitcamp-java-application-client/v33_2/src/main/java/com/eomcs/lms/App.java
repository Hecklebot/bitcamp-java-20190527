...
public class App {
  ...
  private void service() {
    ...
    BoardDao boardDao = new BoardDaoProxy(host, port); // 한 번 연결하고 요청 처리하고 해제하고
    LessonDao lessonDao = new LessonDaoProxy(host, port); // 계속 연결되어 있음
  ...
  public static void main(String[] args) {
    if(args.length != 2) {
      System.out.println("실행방법: java -Dfile.encoding=UTF-8 -cp bin/main com.eomcs.lms.App 서버 주소 포트번호");
      return;
    }
